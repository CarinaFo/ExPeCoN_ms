...
s.file_prefix = 'expeco_';
s.blocks = 5;
s.train_blocks = 1;
s.train_cue_cond_n = [2 2];

%% localizer block
s.localizer_ntrials = 500;
%s.localizer_isi = 2;
s.localizer_isi = [1.4 1.6]; % 2 + (3-2) .* rand(s.localizer_ntrials,1);
s.supra_d = 0.9;
s.supra_name = 'Supra-threshold';

%% resting state block

s.resting_state_time = 300;
...
s.cued_cond_t = 3; % in s
s.feedback_t = 2.5; % in s
s.feedback_cue_t = 1.5;
...
%% Trial
s.get_flip_i = 200;
s.fix_t = [1.000 2.000]; % inter-trial interval in s
s.cue_t = 0.500;
s.stim_delay = 0.4;
s.resp_window = 'variable';
s.resp1_max_t = 2.500; % 1.5
s.resp2_max_t = 2.500; % 1.5
...
s.cue_color = [255 8 0]; %candy apple
