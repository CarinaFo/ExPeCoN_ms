...
def test_mirror_with_tfr_and_plot():
    ...
    epochs = epochs.crop(tmin=-0.7, tmax=0)

    ### Mirroring ###
    mirrored_epochs = zero_pad_or_mirror_epochs(epochs, zero_pad=True, pad_length=pad_length)
    ...
    n_cycles = freqs/4.0 # As a rule of thumb, use n_cycles = freqs / 2
    power_original = mne.time_frequency.tfr_multitaper(epochs, freqs=freqs, n_cycles=n_cycles, return_itc=False, average=True, 
    use_fft=False)
    power_mirrored = mne.time_frequency.tfr_multitaper(mirrored_epochs, freqs=freqs, n_cycles=n_cycles, return_itc=False, average=True, 
    use_fft=False)
    ...
    axes[1].set_title('TFR - Zero padded signal (500ms both ends)')
