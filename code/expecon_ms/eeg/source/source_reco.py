...
def plot_grand_average_source_contrast(study: int, cond: str, method: str, save_plots: bool, backend: str):
    """
    ...
    backend: str, info: which backend to use for plotting
    ...
    if (study == 1) & (cond == "probability"):
        stc_array_hit = create_source_contrast_array(
            study=study, cond_a="high_prevhit_mirror", cond_b="low_prevhit_mirror", method=method
        )
        stc_array_miss = create_source_contrast_array(
            study=study, cond_a="high_prevmiss_mirror", cond_b="low_prevmiss_mirror", method=method
        )
        stc_array_cr = create_source_contrast_array(
            study=study, cond_a="high_prevcr_mirror", cond_b="low_prevcr_mirror", method=method
        )
    ...
    _, p, _ = mne.stats.permutation_t_test(stc_array)  # _, p, _ = t, p, h
