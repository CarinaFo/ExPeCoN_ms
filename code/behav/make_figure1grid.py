...
def plot_figure1_grid():
    ...
    for index in range(len(low_condition_hr)):
        ...
        conf_ax.plot(2, incon_condition.iloc[index,1],
                marker='', markersize=8, color=colors[1], markeredgecolor=colors[1], alpha=.5)
        conf_ax.plot([1,2], [con_condition.iloc[index,1], incon_condition.iloc[index,1]],
                marker='', markersize=0, color='gray', alpha=.25)

    conf_box = conf_ax.boxplot([con_condition.iloc[:,1], incon_condition.iloc[:,1]], patch_artist=True)
...
def stats_figure1():
    ...
    ## stats for Figure 1 (all significant except criterion)

    # non parametric
    t,p = stats.wilcoxon(criterion_high, criterion_low)
    print(f'c: {p}')

    ## stats (all significant except dprime)
