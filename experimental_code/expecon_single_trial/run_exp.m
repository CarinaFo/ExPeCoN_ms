...
%   seq             - experimental trial sequence
...
% don't randomize response buttons for ExPeCoN
...
% vertical display: left top right bottom
...
for i = 1:size(data.seq,1)
    ...
    elseif mod((i-1) ,s.feedback_after_trials) == 0  && s.show_feedback == 1
        ...
        DrawFormattedText(window,s.fix,'center','center',s.txt_color);
        [data.onset_fix_iti{i,:}] = Screen('Flip',window, ...
         data.onset_feedback{i-1,1} + s.feedback_t - flip_t);
    % all other trials
    else
        ...
        DrawFormattedText(window,s.fix,'center','center',s.txt_color);
        [data.onset_fix_iti{i,:}] = Screen('Flip',window,...
         data.onset_resp2{i-1,1} + data.resp2_t(i-1,1) - flip_t);
    end
    ...
    [data.onset_cue_cond{i,:}] = Screen('Flip',window, ...
        data.onset_fix_iti{i,1} + data.seq(i,5) - flip_t); % inter trial interval
   ...
   % Start analog output (triggers waveform immediately, mean 20ms)
    ...
    queueOutputData(aio_s,[data.stim_wave*data.intensity(i,1) ...
        data.stim_wave*0 data.TTL_wave  data.stim_wave*0]);       
    ...
    data.ao_trigger_pre{i,1} = WaitSecs('UntilTime', ...
        data.onset_fix{i,1} + s.stim_delay - (s.pre_pulse_t/1000)); % fixed delay
    ...
    DrawFormattedText(window,data.resp1_txt(1,:), ...
        'center','center',s.txt_color,[],[],[],[],[],resp_rect1);
    DrawFormattedText(window,data.resp1_txt(2,:), ...
        'center','center',s.txt_color,[],[],[],[],[],resp_rect2);
    ...
    [data.resp1_btn(i,1),data.resp1_t(i,1),data.resp1_port(i,:)] = ...
    parallel_button(s.resp1_max_t,data.onset_resp1{i,1},s.resp_window,s.debounceDelay,lpt);        
...
%     end        
    ...
    Screen('TextSize',window,s.txt_size);
%     DrawFormattedText(window,num2str(data.resp2_txt(1,:)), ...
%         'center','center',s.txt_color,[],[],[],[],[],resp_rect3);
    DrawFormattedText(window,num2str(data.resp2_txt(1,:)), ...
        'center','center',s.txt_color,[],[],[],[],[],resp_rect1);
    DrawFormattedText(window,num2str(data.resp2_txt(2,:)), ...
        'center','center',s.txt_color,[],[],[],[],[],resp_rect2);
%     DrawFormattedText(window,num2str(data.resp2_txt(4,:)), ...
%         'center','center',s.txt_color,[],[],[],[],[],resp_rect4);
    
    % 6. show confidence response options on the screen 

    [data.onset_resp2{i,:}] = Screen('Flip',window,data.onset_resp1_p{i,1} ...
        + s.resp_p_min_t - flip_t);
      
    % Wait for key press

    [data.resp2_btn(i,1),data.resp2_t(i,1),data.resp2_port(i,:)] = ...
        parallel_button(s.resp2_max_t,data.onset_resp2{i,1}, ...
        s.resp_window,s.debounceDelay,lpt);       
    ...
    switch data.resp1_btn(i,1)
        ...
        case s.btn_esc
            break
        otherwise
            data.resp1(i,1) = 0; % if participant pressed the wrong button
    end             
...
%                     data.resp2(i,1) = 4; % highly confident
    ...
    if mod(i,s.feedback_after_trials) == 0  && s.show_feedback == 1       
        
        % Calculate detection rates for all intensities in experiment
        % Do not consider trails where no button or the wrong button was
        % pressed
        % 06.04.2023, CF
    ...
    if i == size(data.seq,1)
        ...
        data.wait_block_end = WaitSecs('UntilTime',data.onset_resp2{i,1}...
            + s.resp2_max_t);
    end
