...
%% CUE TO AO TRIGGER
    ...
    data.t_cue_ao_trigger_pre_diff_stim_delay(i,1) = (data.ao_trigger_pre(i,1)-data.onset_cue{i,1}-s.stim_delay)*1000;    


% STIMULUS ONSET LOCKED TO FIRST FIXATION ONSET
data.t_fix_stim_onset = data.ao_trigger_pre{i,1} + data.stim_offset/1000 - data.onset_fix{1,1};

data.t_trigger_ao = data.ao_trigger_post{i,1} - data.ao_trigger_pre{i,1};
end
